...
def procesar_demanda(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Dataframe para comprador no regulado RTQC
    mask1 = df_clean["CodigoSICAgente"] == "RTQC"
    mask2 = df_clean["TipoMercado"] == "No Regulado"
    df_comprador = df_clean[mask1 & mask2]
    df_comprador["Valor"] = df_comprador["Valor"] * 0.1 # Vamos a simular el 10% de la demanda total de un comprador no regulado
