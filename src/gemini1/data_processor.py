...
class DataProcessor:
    ...
    def load_and_process(self):
        ...
        df_spot["FechaHora"] = pd.to_datetime(df_spot["FechaHora"])
        df_spot["Fecha"] = df_spot["FechaHora"].dt.date
        ...
        df_fut["Precio"] = pd.to_numeric(df_fut["Precio"], errors="coerce")
        df_fut.dropna(subset=["Precio"], inplace=True)
        ...
        # Asumimos vencimiento el último día del mes
        ...
        # --- CORRECCIÓN AQUÍ ---
        # Aplanar el MultiIndex de columnas a un índice de tuplas para poder hacer join con Spot
        # Esto convierte las columnas de 2 niveles a 1 nivel de tuplas: [("ELM", date), ...]
        pivot_fut.columns = pivot_fut.columns.to_flat_index()
        
        # Alinear fechas con Spot
        # Ahora ambos tienen 1 nivel de columnas, el join funciona
    ...
    def get_state_for_date(self, full_data, current_date, contract_type):
        ...
        if current_date not in full_data.index:
            return np.zeros(4) # Retornar ceros si no hay datos
            
        row = full_data.loc[current_date]
        spot = row["SpotPrice"]
        
        # Filtrar columnas del contrato específico
        # Como aplanamos el índice, las columnas son tuplas (Tipo, Vencimiento)
        # Buscamos las tuplas donde el primer elemento coincide con el tipo
        ...
        for col_tuple in cols:
            expiration = col_tuple[1] # La fecha está en el segundo elemento de la tupla
        ...
        # Si no hay M2, usamos M1 como referencia para evitar errores, o 0
