"""Evaluación out-of-sample y visualización de resultados PPO (LSTM)."""
...
def execution_metrics(eval_df: pd.DataFrame) -> dict:
    raw_cols = [f"action_disc_raw_{i}" for i in range(1, 7)]
    exec_cols = [f"action_disc_exec_{i}" for i in range(1, 7)]

    raw = eval_df[raw_cols].to_numpy(dtype=np.float32)     # shape (T, 6)
    exe = eval_df[exec_cols].to_numpy(dtype=np.float32)    # shape (T, 6)

    # % de celdas (t,slot) con acción propuesta != 0
    proposed_nonzero_rate = float(np.mean(raw != 0.0))

    # % de celdas (t,slot) con acción ejecutada != 0
    executed_nonzero_rate = float(np.mean(exe != 0.0))

    # % de celdas inválidas: propuesta != ejecutada
    invalid_rate = float(np.mean(raw != exe))

    # opcional: % de pasos donde hubo al menos una acción no-cero
    proposed_step_rate = float(np.mean(np.any(raw != 0.0, axis=1)))
    executed_step_rate = float(np.mean(np.any(exe != 0.0, axis=1)))

    return {
        "proposed_nonzero_rate": proposed_nonzero_rate,
        "executed_nonzero_rate": executed_nonzero_rate,
        "invalid_rate": invalid_rate,
        "proposed_step_rate": proposed_step_rate,
        "executed_step_rate": executed_step_rate,
    }
...
def evaluate_agent_out_of_sample(config: ProjectConfig = CONFIG) -> Dict[str, pd.DataFrame]:
    """Ejecuta evaluación out-of-sample PPO sin ruido (determinística)."""
    ...
    actor_path = weights_dir / "best_actor_ppo.pt"
    ...
    while not (terminated or truncated):
        # PPO determinístico para evaluación
        ...
        covered_kwh = float(sum(pos.quantity_contracts * pos.contract_size_kwh for pos in env_test.inventory.values()))
    ...
    eval_df.to_csv(results_dir / "evaluation_rollout_ppo.csv", index=False)
    spot_daily.to_csv(results_dir / "benchmark_spot_daily.csv", index=False)

    # Métricas de ejecución
    exec_metrics = execution_metrics(eval_df)
    print("\n=== Execution Metrics ===")
...
def build_financial_report(eval_df: pd.DataFrame, spot_daily: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    report = pd.DataFrame(
        {
            ...
            "Costo_Total_Estrategia_PPO_COP": [total_strategy],
...
def plot_learning_curve(results_dir: Path) -> None:
    ...
    history_path = results_dir / "training_history_ppo.csv"
    ...
    ax1.set_title("Curva de Aprendizaje PPO")
    ax1.grid(True, alpha=0.3)

    fig.tight_layout()
    fig.savefig(results_dir / "grafico_1_curva_aprendizaje_ppo.png", dpi=200)
    plt.close(fig)


def plot_monthly_cost_comparison(results_dir: Path, merged: pd.DataFrame) -> None:
    """Gráfico 2: Barras de costo mensual Spot vs PPO."""
    df = merged.copy()
    ...
    monthly = (
        ...
        .sum()
        .rename(columns={"spot_cost": "Escenario_Base_Spot", "strategy_cost_daily": "Estrategia_PPO"})
    )
    ...
    ax.set_title("Comparativa de Costos Mensuales: Spot vs Estrategia PPO")
    ax.set_xlabel("Mes")
    ax.set_ylabel("Costo Total COP")
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig(results_dir / "grafico_2_costos_mensuales_ppo.png", dpi=200)
    plt.close()
...
def evaluate_and_plot(config: ProjectConfig = CONFIG) -> None:
    ...
    _, results_dir = _resolve_dirs(config)
    ...
    report_df.to_csv(results_dir / "financial_report_ppo.csv", index=False)
