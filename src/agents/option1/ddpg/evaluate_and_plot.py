...
def _resolve_dirs(config: ProjectConfig) -> Tuple[Path, Path]:
    ...
    results_dir = Path(getattr(config.paths, "results_dir", "results/option1/ddpg"))
...
def plot_liquidaciones(results_dir: Path, merged: pd.DataFrame) -> None:
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "grafico_3_liquidaciones.png", dpi=200)
    plt.close()
...
def plot_cuenta_margen(results_dir: Path, merged: pd.DataFrame) -> None:
    ...
    ax.legend()
    plt.tight_layout()
    plt.savefig(results_dir / "grafico_4_cuenta_margen.png", dpi=200)
    plt.close()
...
def plot_actions_heatmap(results_dir: Path, eval_df: pd.DataFrame) -> None:
    ...
    plt.tight_layout()
    plt.savefig(results_dir / "grafico_5_mapa_calor_acciones.png", dpi=200)
    plt.close()
