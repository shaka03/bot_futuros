...
def main():
    best_path = Path(CONFIG.paths.results_dir) / "hparam_search" / "best_trial_summary.json"
    best = json.loads(best_path.read_text(encoding="utf-8"))
    hp = json.loads(best["hp_json"])

    cfg = apply_hp(CONFIG, hp)
    cfg = replace(cfg, general=replace(cfg.general, seed=int(best["seed"])))  # opcional: misma seed del best
