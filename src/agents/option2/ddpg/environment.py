...
class ElectricityHedgingEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def step(
        ...
        for month_slot, act in enumerate(discrete_actions, start=1):
            ...
            if act == 1:
                ...
                qty_contracts = int(math.floor((expected_demand_kwh * max(spot_price, 0.0)) / den))
        ...
        if reward > 0:
            ...
            reward = max(reward, -5.0)  # permite castigo más fuerte
    ...
    def _close_position(self, nem: str, close_price: float) -> Tuple[float, float, float]:
        ...
        delta_mtm = (close_price - pos.prev_price) * pos.contract_size_kwh * pos.quantity_contracts
        pnl_close += delta_mtm  # incluye ajuste MtM desde último precio
