...
class ElectricityHedgingEnv(gym.Env[np.ndarray, np.ndarray]):
    ...
    def step(
        ...
        invalid_actions = np.sum(np.abs(discrete_actions - executed_actions))
        ...
        if terminated:
            terminated_reason = "timeline_end"
        elif truncated:
            terminated_reason = "bankruptcy"
        else:
    ...
    def _discretize_action(self, action: np.ndarray) -> np.ndarray:
        """Discretiza acción continua con umbrales."""
